...
public class HeapPage implements Page {
	...
	public HeapPage(HeapPageId id, byte[] data) throws IOException {
		...
		this.data = data;
		//this.numSlot = entryCount();
		this.numSlot = tupleLocation(entryCount());
		setBeforeImage();
	}
	...
	public Iterator<Tuple> iterator() 
	{
        ...
        {
        	//stopping point for when it doesnt exist
			...
			//	for(int i=0; i<tuples.length; i++)
					   // 		if(getTuple(entryID).getTuple(entryID))
				//	    	{
				//	    		tupleList.add(tuples[i]);
				//	    	}
				for (;this.ptr < HeapPage.this.numSlot; this.ptr++)
				{
	...
	public Tuple getTuple(int entryID) {
		...
		if (entryID >= tuples.length)
			throw new NoSuchElementException();

		try {
			if(!isSlotUsed(entryID)) {
				Debug.log(1, "BPlusTreeLeafPage.getTuple: slot %d in %d:%d is not used", entryID, pid.getTableId(), pid.pageNumber());
				return null;
			}

			Debug.log(1, "BPlusTreeLeafPage.getTuple: returning tuple %d", entryID);
			return tuples[entryID];
...
*/
	...
	public Tuple getTuple(int entryID) 
	{
		...
		int x = tupleLocation( entryID );
		//int y = tupleLocation(entryCount());;
		//int y = sizeOfHeader();
		//ByteArrayInputStream byteArray = new ByteArrayInputStream(data, x, BufferPool.PAGE_SIZE/y );
		ByteArrayInputStream byteArray = new ByteArrayInputStream(data, x, data.length - x );
		DataInputStream in = new DataInputStream(byteArray);
		Tuple newTuple = new Tuple(createTuple(in).getTupleDesc());
		newTuple.setRecordId(new RecordId(pid, entryID));
		return newTuple;
		//Tuple newTuple = createTuple(new DataInputStream(new ByteArrayInputStream(data, x, data.length - x ))).getTupleDesc();
		
		//DataInputStream in = new DataInputStream(byteArray);
		
		//Tuple newTuple = createTuple(in);
		//newTuple.getTupleDesc();
		//newTuple.setRecordId(new RecordId(pid, entryID));
		
		//setRecordId(new RecordId(pid, entryID));
		//return newTuple;
	//	throw new UnsupportedOperationException("Implement this");
	    //DataInputStream dat;
		/*
		...
		*/
		...
		try {
            for (int j=0; j<td.numFields(); j++) {
                Field f = td.getType(j).parse(dat);
                newTuple.setField(j, f);
            }
